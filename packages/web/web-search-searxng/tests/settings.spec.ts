...
import { WEB_SEARCH_SEARXNG_SETTINGS_NAMESPACE } from '../src/index.ts'
...
const ONE_RESULT = { results: [{ url: 'https://a.test', title: 'A', content: 'snippet' }] }
...
async function boot(): Promise<{ ctx: Context; settingsFiber: Fiber; pluginFiber: Fiber }> {
  ...
  const pluginFiber = ctx.plugin(searxngPlugin, { baseURL: 'http://composition.test:8888' })
...
describe('web-search-searxng settings section', () => {
  it('serves a stored endpoint to the next search without re-registering the provider', async () => {
    ...
    expect(await searchOnce(bench.ctx)).toContain('http://composition.test:8888/')

    await bench.ctx.settings.update(WEB_SEARCH_SEARXNG_SETTINGS_NAMESPACE, {
      baseURL: 'http://searxng.lan:8888',
    })

    expect(await searchOnce(bench.ctx)).toContain('http://searxng.lan:8888/')
