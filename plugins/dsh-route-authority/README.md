...
Faculty → slot recipes are a **sibling**: `plugins/dsh-route-recipes`. This package only enforces.
...
4. `route-authority.matrix` / compiled recipes / `route-policy` tiers
...
Confirm recipes load first, authority after `dsh-web-app`:

```bash
dsh web --dump-config | rg -n "route-recipes|route-authority|tool-subagent"
```
...
route-authority:
  matrix:
    ...
    rules:
      ...
      - match: { faculty: implement }
        select: { tier: longcat-300k }
```

Prefer editing recipes (`SCHEMA.md` in the sibling) over hand-writing this matrix.
...
Recipe UX, faculty catalogs, and candidate-chain failover.
